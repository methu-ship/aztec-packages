  ...
  describe('with an offline validator', () => {
    ...
    it.only("tries to slash the validator that didn't sign proven blocks", async () => {
      ...
      await retryUntil(
        ...
        'slashing quorum',
        SHORTENED_BLOCK_TIME_CONFIG_NO_PRUNES.aztecSlotDuration * SLASHING_ROUND_SIZE * 2 + 1,
        1,
      );
      ...
      await retryUntil(
        ...
        'slashing executable',
        SHORTENED_BLOCK_TIME_CONFIG_NO_PRUNES.aztecSlotDuration * SLASHING_ROUND_SIZE,
        1,
      );
